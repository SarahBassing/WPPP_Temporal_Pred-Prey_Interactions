  ...
  cat(file = './Outputs/TimeBtwnDetections/tbd_season_predID_X_habitat_nowolf.txt', "
          model{
          ...
          #'  Interaction for TRI * predator species ID (note: one less predator in this model)
          beta3[1] <- 0
          for(jj in 2:4){
            beta3[jj] ~ dnorm(0, 0.01)  
          }
        
          #' #'  Interaction for PercForest * predator species ID (note: one less predator in this model)
          #' beta4[1] <- 0
          #' for(jj in 2:4){
